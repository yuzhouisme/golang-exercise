...
func (oq *OrderQuery) QueryUsers() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2M, true, order.UsersTable, order.UsersPrimaryKey...),
		)
...
func (oq *OrderQuery) sqlAll(ctx context.Context) ([]*Order, error) {
	...
	if query := oq.withUsers; query != nil {
		...
		ids := make(map[int64]*Order, len(nodes))
		...
		var (
			edgeids []uuid.UUID
			edges   = make(map[uuid.UUID][]*Order)
		)
		_spec := &sqlgraph.EdgeQuerySpec{
			...
			Predicate: func(s *sql.Selector) {
				s.Where(sql.InValues(order.UsersPrimaryKey[1], fks...))
			},
